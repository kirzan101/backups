                                        ...
                                        <tbody>
                                            @foreach($replist as $rep)
                                                <tr>
                                                    ...
                                                    <td>
                                                        <div class="form-check">
                                                                <input class="form-check-input position-static cb-perm" type="checkbox" id="read-{{ $rep->id }}" name="report_access[]" value="access-{{ $rep->id }}" aria-label="..." {{ old('report_access') != null && in_array('access-' . $rep->id, old('report_access')) ? 'checked' : '' }}
                                                            
                                                            @foreach ($report_access as $i => $repaccess)
                                                                @if ($repaccess == $rep->id)
                                                                
                                                                    {{ strpos($report_access[$i], '1') !== false ? 'checked' : '' }}
                                                                    OR
                                                                    {{ strpos($report_access[$i], '2') !== false ? 'checked' : '' }}
                                                                    OR
                                                                    {{ strpos($report_access[$i], '3') !== false ? 'checked' : '' }}
                                                                    OR
                                                                    {{ strpos($report_access[$i], '4') !== false ? 'checked' : '' }}
                                                                    OR
                                                                    {{ strpos($report_access[$i], '5') !== false ? 'checked' : '' }}
                                                                    OR
                                                                    {{ strpos($report_access[$i], '6') !== false ? 'checked' : '' }}
                                                                    @break
                                                                @endif
                                                            @endforeach
                                                            >
